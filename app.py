...
# Constants
# Try getting from Streamlit secrets first, then environment, then fallback (but don't hardcode in public repo)
try:
...
# Page Configuration
st.set_page_config(page_title="Pizza Sales Dashboard & AI Assistant", page_icon="🍕", layout="wide")

# Check for secrets
if not QDRANT_API_KEY or not HF_API_KEY:
    st.error("Missing API Keys! Please configure .streamlit/secrets.toml or environment variables.")
    st.stop()
...
# Tabs for Dashboard vs AI Chat
tab_dashboard, tab_chat = st.tabs(["📊 Dashboard", "🤖 AI Assistant (RAG)"])
...
with tab_chat:
    st.header("🤖 AI Business Assistant (RAG)")
    st.markdown("Ask questions about your pizza sales, and I'll analyze the data using 'Kimi K2' (via Hugging Face) and our Vector Database.")
    ...
    # Chat input
    if prompt := st.chat_input("Ex: What are the top selling pizzas in July?"):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                # 3. Construct Context
                # Add High-Level Summary Context to solve aggregation issues
                if df is not None:
                    ...
                    # Top 5 Pizzas
                    top_5 = df.groupby('pizza_name')['quantity'].sum().sort_values(ascending=False).head(5)
                    top_5_str = ", ".join([f"{name} ({qty})" for name, qty in top_5.items()])
                    
                    # Category Sales
                    cat_sales = df.groupby('pizza_category')['quantity'].sum()
                    cat_sales_str = ", ".join([f"{cat}: {qty}" for cat, qty in cat_sales.items()])
                    ...
                    - Top 5 Best Selling Pizzas: {top_5_str}
                    - Sales by Category: {cat_sales_str}
                    """
                else:
                    summary_context = "Global statistics not available."

                context_text = f"--- Global Summary ---\n{summary_context}\n\n--- Retrieved Specific Orders (for detailed context) ---\n"
                for result in search_results:
                    context_text += f"- {result.payload}\n"
                
                # 4. Construct Prompt for LLM
                system_prompt = "You are a helpful business analyst for a pizza restaurant. You have access to a Global Dataset Summary and a few specific retrieved order examples. ALWAYS use the Global Summary for questions about totals, averages, or top sellers. Only use the retrieved orders for questions about specific transaction details. If the answer isn't in the context, say you don't know."
                full_prompt = f"Context:\n{context_text}\n\nQuestion: {prompt}\n\nAnswer:"
                
                # 5. Call Hugging Face API
                client = get_hf_client()
                
                # Using chat completion API if supported, or text generation
                # Some models on HF Inference API work better with raw text generation or specific chat templates
                # We'll try a standard chat format if the model supports it, otherwise raw text
